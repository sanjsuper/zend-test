...
class UserExistsValidator extends AbstractValidator
{
    ...
    protected $messageTemplates = array(
        self::NOT_SCALAR => "The login must be a scalar value",
        self::USER_EXISTS => "Another user with such an login already exists"
    );
    ...
    public function isValid($value)
    {
        ...
        if ($this->options['user'] == null) {
            ...
            if ($this->options['user']->getLogin() != $value && $user != null)
                $isValid = false;
            else
                $isValid = true;
        }
